...
import { Robotz } from "../typechain-types";

describe("🚩 Robotz: 🤖 NFT Example 🤓", function () {
  let myContract: Robotz;

  describe("Robotz", function () {
    const contractAddress = process.env.CONTRACT_ADDRESS;

    let contractArtifact: string;
    if (contractAddress) {
      // For the autograder.
      contractArtifact = `contracts/download-${contractAddress}.sol:Robotz`;
    } else {
      contractArtifact = "contracts/Robotz.sol:Robotz";
    }

    it("Should deploy the contract", async function () {
      const Robotz = await ethers.getContractFactory(contractArtifact);
      myContract = await Robotz.deploy();
